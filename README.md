...
「迷ったら委譲しない」「委譲は手段であって工程ではない」を基本にする。state machine、強制 planner、agent counter は持たない。持つ状態は、ユーザーが明示的に ON にした委譲モードのフラグ（セッション単位）だけ。
...
## 委譲モード: /ensemble:on と /ensemble:off

`/ensemble:run` を、セッションの間ずっと効かせたいときに使う。

```
/ensemble:on    # 委譲モードを ON にする
/ensemble:off   # OFF に戻す
```

- ON の間は、毎ターン短い指示（specialist ごとの使いどころと「小さな修正は直接やる」）をプロンプトに足す。長い会話で委譲が弱まるのを防ぐ
- OFF のときは何も足さない。普段どおり「迷ったら委譲しない」で動く
- 状態はセッション単位。新しいセッションは OFF から始まる。`--resume` で再開したセッションは ON のまま続く
- 持つ状態は、OS の一時ディレクトリ（`<tmp>/claude-ensemble/mode/`）に置くセッションごとの空ファイル1つだけ。プロジェクトには何も書かない。7日以上前のものは次に ON にしたときに消す
- 固定の工程は回さない。何を委譲するかは、ON の間も毎ターンその場で判断する
- ユーザー専用。Claude が自分から ON / OFF することはない
